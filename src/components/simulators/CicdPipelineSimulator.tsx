...
import { generateCicdPipelineSteps, type PipelineNode, type PipelineNodeStatus } from "@/lib/simulators/cicdPipelineSteps";
...
export default function CicdPipelineSimulator() {
  const steps = useMemo(() => generateCicdPipelineSteps(), []);
            ...
            return (
              <div key={n.id} className="absolute" style={{ left: p.left, top: p.top }}>
                <div
                  ...
                  style={{ width: NODE_SIZE + 30, height: NODE_SIZE, backgroundColor: STATUS_COLOR[n.status] }}
                >
                  {n.label}
                </div>
                <p className="mt-1 w-20 text-center text-[10px] uppercase leading-tight text-ink-faint">{n.status}</p>
              </div>
            );
          })}
        </div>
